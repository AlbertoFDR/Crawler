 ...
 * @param {object} results Results object to save the data
 * @param {object} Network Network object to download the file
 */
function loadingFinished(params, results, Network) {
    debug("Entering Network.loadingFinished handler");

    for (i = 0; i < results.requests.length - 1; i++) {
        if (params.requestId == results.requests[i].requestId) {
            // Save the total data
            ...
            // Check if is a file type of the config
            if (results.requests[i].type && config.filesToDownload.includes(results.requests[i].type)) {
                // Download the file
                Network.getResponseBody({
                    ...
                    .then((file) => {
                        ...
                        if (file.base64Encoded) {
                            ...
                            //Write file (Not base64 encoded)
                            fs.writeFile(path, file.body, (err) => {
                                if (err) throw err;
                            });
                        }
                    })
